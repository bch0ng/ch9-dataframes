...
salary2015 <- runif(100, 40000, 90000)

# Create a data.frame 'salaries' by combining the vectors you just made
salaries <- data.frame(employees, salary2014, salary2015)

# Create a column 'raise' that stores the size of the raise between 2014 and 2015
salaries$raise <- (salary2015 - salary2014)

# Create a column 'got.raise' that is TRUE if the person got a raise
salaries$got.raise <- (salary2015 > salary2014)
...
sum(salaries$got.raise, na.rm = TRUE)
...
salaries$employees[salaries$raise == min(salaries$raise)]

# What was the average salary increase?
mean(salaries$raise[salaries$got.raise], na.rm = TRUE)
